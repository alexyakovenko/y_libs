...
const char const * YFF1A_FILE_NAME="/home/ayakovenko/y_project/top/yff1.a";
const char const * YFF1P_FILE_NAME="/home/ayakovenko/y_project/top/yff1.p";
const char const * GAFFA_FILE_NAME="/home/ayakovenko/y_project/top/gaff.a";
const char const * GAFFB_FILE_NAME="/home/ayakovenko/y_project/top/gaff.b";
const char const * GAFFG_FILE_NAME="/home/ayakovenko/y_project/top/gaff.g";
const char const * GAFFI_FILE_NAME="/home/ayakovenko/y_project/top/gaff.i";
const char const * GAFFD_FILE_NAME="/home/ayakovenko/y_project/top/gaff.d";
const char const * YFF1R_FILE_NAME="/home/ayakovenko/y_project/top/yff1.r";
...
FILE *in[0x8];
t_top *top=0x0;
if (!(in[0x0]=fopen(YFF1A_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file yff1.a for reading"); ylib_errno=YERROR_USER; return FALSE; }
if (!(in[0x1]=fopen(YFF1P_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file yff1.p for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_0; }
if (!(in[0x2]=fopen(GAFFA_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file gaff.a for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_1; }
if (!(in[0x3]=fopen(GAFFB_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file gaff.b for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_2; }
if (!(in[0x4]=fopen(GAFFG_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file gaff.g for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_3; }
if (!(in[0x5]=fopen(GAFFI_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file gaff.i for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_4; }
if (!(in[0x6]=fopen(GAFFD_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file gaff.d for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_5; }
if (!(in[0x7]=fopen(YFF1R_FILE_NAME,"r"))) { yprintf(YPRINTF_ERROR,"Can't open file yff1.r for reading"); ylib_errno=YERROR_USER; goto LABEL_ERROR_6; }
...
//This function reads first structure from sdf file.
//Count is the line # returned in case of error
t_str *read_sdf(unsigned int *count,FILE *in)
{
register unsigned int _i, _j, _k;
char *string, *lexem, *name=0x0;
t_str *str=0x0;
extern char buffer[];

//Stage I. Read the header (3 lines)
if (!(string=yfgets(in))) return FALSE; else { if ( (count)) (*count)++; }
if ( (lexem=get_lex(0x1,string))) //No mol name otherwise, this is permitted but not desired
  {
  _i=lexlen(string);
  if (!(name=(char*)malloc(sizeof(char)*(_i+1)))) { ylib_errno=YERROR_MEMORY; return FALSE; } else { name[_i]='\0'; memcpy(name,lexem,sizeof(char)*_i); }
  }
if (string!=buffer) { free(string), string=0x0; }  
if ( (!(fstrskip(in)))||(!(fstrskip(in))) ) { ylib_errno=YERROR_DATA_FORMAT; return FALSE; } else { if ( (count)) (*count)+=2; } 
//Stage II. Read record data string
if (!(string=yfgets(in))) { if ( (name)) { free(name), name=0x0; } return FALSE; }
if ( (lexem=get_lex(0x1,string))) lexem[_j=lexlen(lexem)]='\0';
else { LABEL_DATA_FORMAT_0: ylib_errno=YERROR_DATA_FORMAT; 
     if ( (name))        { free(name),     name=0x0; }
     if (string!=buffer) { free(string), string=0x0; } return FALSE; }
if ( ( (check_uint_type(lexem)))&&( (_i=(unsigned int)atoi(lexem)))&&( (lexem=get_lex(0x1,&lexem[_j+1]))) ) lexem[_j=lexlen(lexem)]='\0';
else goto LABEL_DATA_FORMAT_0;
if ( (!(check_uint_type(lexem)))||(!(_j=(unsigned int)atoi(lexem))) ) goto LABEL_DATA_FORMAT_0;
if (string!=buffer) { free(string), string=0x0; }
if (!(str=alloc_str(0x0,1,_i,_j))) { if (name) { free(name), name=0x0; } return FALSE; }
...
for (_k=0;_k<str->natoms;_k++)
  if (!(string=yfgets(in))) { LABEL_DATA_FORMAT_1: free_str(str), str=0x0; ylib_errno=YERROR_DATA_FORMAT; return FALSE; }
  else
    {
    if (strlen(string)<34) { LABEL_DATA_FORMAT_2: if (string!=buffer) { free(string), string=0x0; } goto LABEL_DATA_FORMAT_1; }
    else string[10]=string[20]=string[30]=string[34]='\0'; 
    if ( (!(check_double_type(&string[01])))||(!(check_double_type(&string[11])))||(!(check_double_type(&string[21])))||(!(str->a[_k]=name_to_chemid(&string[31]))) )
      goto LABEL_DATA_FORMAT_2; 
    MAKE_INT_NAME(_i,_j,((char*)str->anames[_k]),((char*)&string[31]));
    str->r[_k].i=atof(&string[01]), str->r[_k].j=atof(&string[11]), str->r[_k].k=atof(&string[21]);
    if ( (count)) (*count)++;
    if (string!=buffer) { free(string), string=0x0; }
    }
//Stage IV. Read bonds
for (_k=0;_k<str->nedges;_k++)
  if (!(string=yfgets(in))) goto LABEL_DATA_FORMAT_1;
  else
    { 
    if (strlen(string)<9) goto LABEL_DATA_FORMAT_2; else string[9]='\0';
    if (!(name=get_lex(0x1,&string[6])))  goto LABEL_DATA_FORMAT_2;
    switch (name[0]) //define bond type from lexem type
      ...
      case '2' :
      case '3' : {
                 if (lexlen(name)!=1) goto LABEL_DATA_FORMAT_2;
                 str->edges[_k].type=(int)name[0];
                 break;
                 }
      case 'a' : {
                 if (lexlen(name)!=2) goto LABEL_DATA_FORMAT_2;
                 switch (name[1])
                   {
                   case 'm' : {//Amide bond 
                   ...
                   case 'r' : {//Aromatic bond
                    ...
                    default  : goto LABEL_DATA_FORMAT_2;
                    }
                 }
      default : goto LABEL_DATA_FORMAT_2;
      }
    string[6]='\0';
    if ( (!(check_uint_type(&string[3])))||((str->edges[_k].vertice[1]=atoi(&string[3]))>str->natoms) ) goto LABEL_DATA_FORMAT_2; else str->edges[_k].vertice[1]--;
    string[3]='\0';
    if ( (!(check_uint_type(&string[0])))||((str->edges[_k].vertice[0]=atoi(&string[0]))>str->natoms) ) goto LABEL_DATA_FORMAT_2; else str->edges[_k].vertice[0]--;
    if ( (count)) (*count)++;
    if (string!=buffer) { free(string); string=0x0; }
    }
...
//Write X-[N, O, P, S]-H_all constraints if necessary
flag=FALSE; _i=mol->natoms;
while (_i--)
  if ( ( (neighbors->list[_i].size))&&(neighbors->list[_i].size!=1)&&
     ( (mol->a[_i]==CHEM_ATOM_TYPE_NITROGEN)||(mol->a[_i]==CHEM_ATOM_TYPE_OXYGEN)||(mol->a[_i]==CHEM_ATOM_TYPE_PHOSPHOR)||(mol->a[_i]==CHEM_ATOM_TYPE_SULFUR) ) )
    {
    amber_atom_type=(unsigned int)-1, _j=neighbors->list[_i].size;
    while (_j--) if (mol->a[neighbors->list[_i].list[_j]]!=CHEM_ATOM_TYPE_HYDROGEN) { if (amber_atom_type==(unsigned int)-1) amber_atom_type=_j; else { amber_atom_type=(unsigned int)-1; break; } }
    if (amber_atom_type!=(unsigned int)-1)
      {//Write the constraint(s)
      if (!(flag)) { flag=TRUE; if (fprintf(out,"\n[ constraints ]\n")<0) goto LABEL_IO_ERROR; }
      //Get X - [N, O, P, S] bond lenght
      _l=mol->size_b; do _l--; while ( ( (mol->ff_b[_l].atom[0]!=_i)||(mol->ff_b[_l].atom[1]!=neighbors->list[_i].list[amber_atom_type]) )&&
                                       ( (mol->ff_b[_l].atom[0]!=neighbors->list[_i].list[amber_atom_type])||(mol->ff_b[_l].atom[1]!=_i) ) ) ; a=mol->ff_b[_l].v; 
      _j=neighbors->list[_i].size;
      while (_j--)
        if (_j!=amber_atom_type)
          {//Get [N, O, P, S] - H(j_th) bond lenght
          _l=mol->size_b; do _l--; while ( ( (mol->ff_b[_l].atom[0]!=_i)||(mol->ff_b[_l].atom[1]!=neighbors->list[_i].list[_j]) )&&
                                                ( (mol->ff_b[_l].atom[0]!=neighbors->list[_i].list[_j])||(mol->ff_b[_l].atom[1]!=_i) ) ) ; b=mol->ff_b[_l].v; 
          //Get X - [N, O, P, S] - H(j_th) angle value
          _l=mol->size_g; do _l--; while ( (mol->ff_g[_l].atom[1]!=_i)||
                                       ( ( (mol->ff_g[_l].atom[0]!=neighbors->list[_i].list[_j])||(mol->ff_g[_l].atom[2]!=neighbors->list[_i].list[amber_atom_type]) )&&
                                         ( (mol->ff_g[_l].atom[0]!=neighbors->list[_i].list[amber_atom_type])||(mol->ff_g[_l].atom[2]!=neighbors->list[_i].list[_j]) ) ) ) ;
          b=0.1*sqrt(a*a+b*b-2.*a*b*cos(mol->ff_g[_l].v));
          if (fprintf(out,"%5d  %5d  1  %12.8e  %12.8e\n",neighbors->list[_i].list[amber_atom_type]+1,neighbors->list[_i].list[_j]+1,b,b)<0) goto LABEL_IO_ERROR;
          }
      }
    }
...
{
register unsigned int _i, _j;
t_clist *_neighbors;  //The temporary list of heavy atom neighbors
char (*_order)[4];  //The temporary order of heavy atom 
void *vp;
...
if ( (verbose))
  ...
  if (!(_order=(char(*)[4])define_neighbors_order(0x0,str->natoms,str->nedges,str->edges)))
    { yprintf(YPRINTF_ERROR,"error in define_neighbors_order(), err_code=%s.\n",get_yerrno(ylib_errno)); free(_neighbors), _neighbors=0x0; return FALSE; }
  if ((_i=update_NO2(_order,str))==(unsigned int)-1) 
    {  yprintf(YPRINTF_ERROR,"error in update_NO2(), err_code=%s.\n",get_yerrno(ylib_errno)); LABEL_ERROR_1: free(_neighbors), _neighbors=0x0; free(_order), _order=0x0; return FALSE; }
  else yprintf(YPRINTF_INFO,"update_NO2() has successfully edited %d NO2 groups.\n",_i);
  if ((_i=update_amides(_order,str))==(unsigned int)-1) 
  ...
  if (pH<=13.6)
    {
    if ((_i=update_guanidines(&_order,_neighbors,str))==(unsigned int)-1) 
  ...
  if (pH<=12.0)
    {
    if ((_i=update_amines(&_order,_neighbors,str))==(unsigned int)-1) 
  ...
  if (pH>=3.0)
    {
    if ((_i=update_carboxyles(_order,str))==(unsigned int)-1) 
  ...
  if (pH>=4.0) 
    {
    if ((_i=update_silicates(_order,str))==(unsigned int)-1) 
  ...
  if (pH>=2.0)
    {
    if ((_i=update_phosphates(_order,str))==(unsigned int)-1) 
  ...
  if (pH>=2.0)
    {
    if ((_i=update_sulfaites(_order,str))==(unsigned int)-1) 
  ...
  if (!(_neighbors=define_neighbors(FALSE,str->natoms,str->nedges,str->edges))) 
    {  yprintf(YPRINTF_ERROR,"error in define_neighbors(), err_code=%s.\n",get_yerrno(ylib_errno)); goto LABEL_ERROR_1; }
  else arrange_neighbors(_order,_neighbors,str->natoms,str->nedges,str->edges);
  if ((_i=update_double_bonds(_order,_neighbors,str))==(unsigned int)-1)
    { 
    yprintf(YPRINTF_ERROR,"error in update_double_bonds(), err_code=%s.\n",get_yerrno(ylib_errno));
    LABEL_ERROR_2: free(_neighbors), _neighbors=0x0; free(_order), _order=0x0; goto LABEL_ERROR_1;
    }
  ...
  if (!(_order=(char(*)[4])define_neighbors_order(0x0,str->natoms,str->nedges,str->edges))) { free(_neighbors), _neighbors=0x0; return FALSE; }
  if ((_i=update_NO2(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
  if ((_i=update_amides(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
  if (pH<=13.6)
    {
    if ((_i=update_guanidines(&_order,_neighbors,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    }
  if (pH<=12.0)
    {
    if ((_i=update_amines(&_order,_neighbors,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    }
  if (pH>=3.5)
    {
    if ((_i=update_carboxyles(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    }
  if (pH>=2.0)
    {
    if ((_i=update_phosphates(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    } 
  if (pH>=4.0) 
    {
    if ((_i=update_silicates(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    }  
  if (pH>=2.0)
    {
    if ((_i=update_sulfaites(_order,str))==(unsigned int)-1) goto LABEL_ERROR_1;
    }
  free(_neighbors), _neighbors=0x0;
  if (!(_neighbors=define_neighbors(FALSE,str->natoms,str->nedges,str->edges))) goto LABEL_ERROR_1;
  else arrange_neighbors(_order,_neighbors,str->natoms,str->nedges,str->edges);
  if ((_i=update_double_bonds(_order,_neighbors,str))==(unsigned int)-1) goto LABEL_ERROR_2;
  }
...
if ( (neighbors)) (*neighbors)=_neighbors; else { free(_neighbors), _neighbors=0x0; }
if ( (order))         (*order)=_order;     else { free(_order),         _order=0x0; }

return TRUE;
}

//This function creates a massive of (*char)[sizeof(int)] to denote | atom_type | 0xFF ^ n | ... |
unsigned int *calc_brutto_f(unsigned int natoms,char *a)
{
unsigned int *brutto_f;
register unsigned int _i, _t;
unsigned int size=0;
unsigned int ch[0xFF]={ 0 };
while (natoms--) if (!ch[(unsigned int)((unsigned char)a[natoms])]++) { if (!(a[natoms])) { ylib_errno=YERROR_INTERNAL_CODE; return FALSE; } else size++;  }
if (!(brutto_f=(unsigned int*)malloc(sizeof(unsigned int)*(1+size)))) { ylib_errno=YERROR_MEMORY; return FALSE; } 
*brutto_f=_i=0;
LABEL_LOOP: ++_i;
if ( (ch[_i]))
  {
       if (sizeof(int) ==  sizeof(char))
         { ylib_errno=YERROR_LEGAL; free(brutto_f), brutto_f=0x0; return FALSE; }
  else if ( sizeof(int) == 4 * sizeof(char) )
         {
         if (ch[_i]>0xFFFFFF) { ylib_errno=YERROR_LEGAL; free(brutto_f), brutto_f=0x0; return FALSE; }
         ++(*brutto_f), ((char*)&brutto_f[*brutto_f])[3]=_i, ((char*)&brutto_f[*brutto_f])[2]=ch[_i]/0xFFFF, ((char*)&brutto_f[*brutto_f])[1]=(ch[_i]%0xFFFF)/0xFF, ((char*)&brutto_f[*brutto_f])[0]=(ch[_i]%0xFFFF)%0xFF;
        }
  else  {
        if (ch[_i]>(1<<(8*(sizeof(int)/sizeof(char)-1)))-1) { ylib_errno=YERROR_LEGAL; free(brutto_f), brutto_f=0x0; return FALSE; }
        ++(*brutto_f), ((char*)&brutto_f[*brutto_f])[_t=sizeof(int)/sizeof(char)-1]=_i; while (_t--) { ((char*)&brutto_f[*brutto_f])[_t]=ch[_i]/(1<<8*_t), ch[_i]%=(1<<8*_t); }
        }
  if (*brutto_f==size) return brutto_f;
  }
goto LABEL_LOOP;
}
...
int item;
size_t len;
for (len=0x0, _i=0; _i!=*brutto_f; _i++) 
  { 
  item=chemid_to_name(((char*)&brutto_f[_i])[sizeof(int)/sizeof(char)-1]);
       if (sizeof(int) == sizeof(char)) 
         { ylib_errno=YERROR_LEGAL; return (unsigned int)-1; }
  else if (sizeof(int) == sizeof(char) * 0x4)
         { item=((char*)&brutto_f[_i+1])[3], _j=((char*)&brutto_f[_i+1])[2]*0xFFFF+((char*)&brutto_f[_i+1])[1]*0xFF+((char*)&brutto_f[_i+1])[0]; }
  else   { item=((char*)&brutto_f[_i+1])[_t=sizeof(int)/sizeof(char)-1], _j=0; while (_t--) _j+=((char*)&brutto_f[_i+1])[_t]*(1<<8*_t);          }
  len+=snprintf(0x0,0,"%s%d",(const char*)&item,_j);
  }
if (!((*brt_f)=(char*)malloc(sizeof(char)*(len+1)))) { ylib_errno=YERROR_MEMORY; return (unsigned int)-1; }
for (len=0x0, _i=0; _i!=*brutto_f; _i++)
  { 
  item=chemid_to_name(((char*)&brutto_f[_i])[sizeof(int)/sizeof(char)-1]);
       if (sizeof(int) == sizeof(char)) 
         { ylib_errno=YERROR_LEGAL; free(*brt_f), *brt_f=0x0; return (unsigned int)-1; }
  else if (sizeof(int) == sizeof(char) * 0x4)
         { item=((char*)&brutto_f[_i+1])[3], _j=((char*)&brutto_f[_i+1])[2]*0xFFFF+((char*)&brutto_f[_i+1])[1]*0xFF+((char*)&brutto_f[_i+1])[0]; }
  else   { item=((char*)&brutto_f[_i+1])[_t=sizeof(int)/sizeof(char)-1], _j=0; while (_t--) _j+=((char*)&brutto_f[_i+1])[_t]*(1<<8*_t);          }
  len+=sprintf(&(*brt_f)[len],"%s%d",(const char*)&item,_j);
  }
(*brt_f)[len]='\0';
return len+1;
}
...
//This function edits single bonds in aromaic rings, i.e. '1' -> 'a'
unsigned int update_aromatic_cycles_bonds(char (*order)[4],t_clist *neighbors,unsigned int size_ar,t_clist *cycles,t_str *str)
{
register unsigned int _i, _j, _k, _l, _n, _t;
unsigned int count=0;
while (size_ar--)
  {
  _i=cycles->list[size_ar].size;
  while (_i--)
    {
    _j=cycles->list[size_ar].list[_i], _n=order[_j][0], _l=str->nedges;
    while ( (_n))
      {
      if (--_l==(unsigned int)-1) { ylib_errno=YERROR_INTERNAL_CODE; return (unsigned int)-1; } //out of bo(u)nds :)
      if (str->edges[_l].type=='1')
        {
        if (str->edges[_l].vertice[0]==_j)
          {
          _t=str->edges[_l].vertice[1]; 
          CHECK_EDGE: _n--;
          if (find_in_row(_t,cycles->list[size_ar].size,cycles->list[size_ar].list)!=(unsigned int)-1)
            {
            str->edges[_l].type='a', count++;
            //Handle j-th vertice
            if (neighbors->list[_j].list[(order[_j][3]+order[_j][2]+order[_j][1])]!=_t)
              {//Rearrange neighbors
              if ((_k=find_in_row(_t,(unsigned int)order[_j][0],&neighbors->list[_j].list[(order[_j][3]+order[_j][2]+order[_j][1])]))==(unsigned int)-1) { ylib_errno=YERROR_INTERNAL_CODE; return (unsigned int)-1; }
              else { neighbors->list[_j].list[(order[_j][3]+order[_j][2]+order[_j][1]+_k)]=neighbors->list[_j].list[(order[_j][3]+order[_j][2]+order[_j][1])], neighbors->list[_j].list[(order[_j][3]+order[_j][2]+order[_j][1])]=_t; } 
              }
            order[_j][0]--, order[_j][1]++;
            //Handle t-th vertice
            if (neighbors->list[_t].list[(order[_t][3]+order[_t][2]+order[_t][1])]!=_j)
              {//Rearrange neighbors
              if ((_k=find_in_row(_j,(unsigned int)order[_t][0],&neighbors->list[_t].list[(order[_t][3]+order[_t][2]+order[_t][1])]))==(unsigned int)-1) { ylib_errno=YERROR_INTERNAL_CODE; return (unsigned int)-1; }
              else { neighbors->list[_t].list[(order[_t][3]+order[_t][2]+order[_t][1]+_k)]=neighbors->list[_t].list[(order[_t][3]+order[_t][2]+order[_t][1])], neighbors->list[_t].list[(order[_t][3]+order[_t][2]+order[_t][1])]=_j; } 
              }
            order[_t][0]--, order[_t][1]++;
            } 
          }
        else if (str->edges[_l].vertice[1]==_j) { _t=str->edges[_l].vertice[0]; goto CHECK_EDGE; }
        }
      }
    }
  }
  ...
  if ( (neighbors->list[_i=mol->edges[_t].vertice[0]].size!=1)&&(neighbors->list[_j=mol->edges[_t].vertice[1]].size!=1) )
    {
         if (anchor_id[_i]!=anchor_id[_j]) //Rule 2.1. Cross-anchor
           ...
           {
           //Rule 2.0. Y~X-(H)2
           if ( ( (neighbors->list[_i].size==3)&&(order[_i][0]==2)&&(neighbors->list[_i].list[0]==_j)&&(mol->a[neighbors->list[_i].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)&&(mol->a[neighbors->list[_i].list[2]]==CHEM_ATOM_TYPE_HYDROGEN) )||
                ( (neighbors->list[_j].size==3)&&(order[_j][0]==2)&&(neighbors->list[_j].list[0]==_i)&&(mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)&&(mol->a[neighbors->list[_j].list[2]]==CHEM_ATOM_TYPE_HYDROGEN) ) ) 
             { mol->size_d+=(neighbors->list[_i].size-1)*(neighbors->list[_j].size-1); continue; } //Symmetric H2 case - use dihedral quasy-improper instead
...
while (_t--)
  ...
  if ( (neighbors->list[_i].size!=1)&&(neighbors->list[_j].size!=1) )
    ...
    else if ( (mol->edges[_t].type!='1')&&
            ( ( (neighbors->list[_i].size==3)&&(order[_i][0]==2)&&(neighbors->list[_i].list[0]==_j)&&(mol->a[neighbors->list[_i].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)&&(mol->a[neighbors->list[_i].list[2]]==CHEM_ATOM_TYPE_HYDROGEN) )||
              ( (neighbors->list[_j].size==3)&&(order[_j][0]==2)&&(neighbors->list[_j].list[0]==_i)&&(mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)&&(mol->a[neighbors->list[_j].list[2]]==CHEM_ATOM_TYPE_HYDROGEN) ) ) ) 
           {//Symmetric H2 case - use dihedral quasy-improper instead. Quasy-dihedrals
           _k=neighbors->list[_i].size;
           while (_k--)
             if (neighbors->list[_i].list[_k]!=_j)
               ...
               _l=neighbors->list[_j].size;
               while (_l--)
                 if ((neighbors->list[_j].list[_l]!=_i)&&(neighbors->list[_j].list[_l]!=neighbors->list[_i].list[_k]))
                   {   
                   mol->ff_d[mol->size_d].atom[0]=neighbors->list[_i].list[_k], mol->ff_d[mol->size_d].atom[1]=_i, mol->ff_d[mol->size_d].atom[2]=_j, mol->ff_d[mol->size_d].atom[3]=neighbors->list[_j].list[_l];
                   mol->ff_d[mol->size_d].d=(neighbors->list[_i].size-1)*(neighbors->list[_j].size-1), mol->ff_d[mol->size_d].k[0]=(double)NAN, mol->ff_d[mol->size_d].v[0]=0., mol->ff_d[mol->size_d].n[0]=2., mol->size_d++;
                   }
               }
           } 
               ...
               if ( (mol->cycles->list[_l].size<=8)&&(find_in_list(_i,&mol->cycles->list[_l])!=(unsigned int)-1)&&(find_in_list(_j,&mol->cycles->list[_l])!=(unsigned int)-1) )           
                 ...
                 if (neighbors->list[_i].list[1]==_j)
                   ...
                   if (mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)
                     ...
                     {//'H' == mol->a[neighbors->list[_j].list[2]]
                     if (neighbors->list[_j].list[0]==_i) 
                       {
                       ...
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N -H
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[2], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N -H
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N-R2
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[2], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N-R2
                       }
                     }
                   }
                 else
                   ...
                   if (mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)
                     ...
                     if (neighbors->list[_j].list[0]==_i) 
                       ...
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[1], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N -H
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N-R2
                       mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[1], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N-R2
                       }
                     }
                   }
                 goto NEXT_BOND;
             ...
             if (neighbors->list[_i].list[1]==_j)
               ...
               if (mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)
                 ...
                 if (neighbors->list[_j].list[0]==_i) 
                   ...
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N -H
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[2], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N -H
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N-R2
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[2], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N-R2
                   }
                 }
               }
             else
               ...
               if (mol->a[neighbors->list[_j].list[1]]==CHEM_ATOM_TYPE_HYDROGEN)
                 ...
                 if (neighbors->list[_j].list[0]==_i) 
                   ...
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[1], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[2], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N -H
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[0], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=0., mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //O -C-N-R2
                   mol->ff_i[mol->size_i].atom[0]=neighbors->list[_i].list[1], mol->ff_i[mol->size_i].atom[1]=_i, mol->ff_i[mol->size_i].atom[2]=_j, mol->ff_i[mol->size_i].atom[3]=neighbors->list[_j].list[0], mol->ff_i[mol->size_i].type=4, mol->ff_i[mol->size_i].v=PI, mol->ff_i[mol->size_i].k=(double)NAN, mol->size_i++; //R1-C-N-R2
                   }
                 }
               goto NEXT_BOND;
               }
...
while (_i--)
  if ( (!gtypes[_i])&&(mol->a[_i]==CHEM_ATOM_TYPE_NITROGEN)&&(!order[_i][3])&&(!order[_i][2]) )
    {
    if ( (order[_i][1])) { if ( (inacycles->list[_i].size)) gtypes[_i]='a'; else gtypes[_i]=' '; }
    else 
      {
...
//Stage I. Scale angles at quasy-impropers so they match PI all-together
//The solutions of conditional extreme (minima) with Lagrange multipliers yields v[i] = v0[i] + (2*PI-SUMM[j](v0[j])) / (k[i]*SUMM[j](1./k[j])) 
//Lagrange f(a,b,g):=k1*(a-a0)^2+k2*(b-b0)^2+k3*(g-g0)^2
//Lagrange p(a,b,g):= PI-a-b-g==0
//Lagrange f(a,b,g)+l*p(a,b,g) -> min (extremum)
// | k1  0  0  -1 |   | a  |   | k1*a0 |
// |  0 k2  0  -1 | . | b  | = | k2*b0 |, where l`=l/2. <- formal Lagrange multiplier
// |  0  0 k3  -1 |   | g  |   | k3*g0 |
// |  1  1  1   0 |   | l` |   | PI    |
_l=mol->size_d;
while (_l--)
  if ( (!(mol->ff_d[_l].d))&&(neighbors->list[mol->ff_d[_l].atom[0]].size==3)&&(fabs(fmod(mol->ff_d[_l].v[0],PI))<SMALL) )
    {//Quasy improper
    _j=_k=(unsigned int)-1, _i=mol->size_g;
    while (TRUE)
      {
      if (!_i) { ylib_errno=YERROR_INTERNAL_CODE; return (unsigned int)-1; } else _i--;
      if (mol->ff_g[_i].atom[1]==mol->ff_d[_l].atom[0]) { if (_k==(unsigned int)-1) _k=_i; else if (_j==(unsigned int)-1) _j=_i; else break; }
      }
    a.alpha=2.*PI-mol->ff_g[_i].v-mol->ff_g[_j].v-mol->ff_g[_k].v, _d=1./mol->ff_g[_i].k+1./mol->ff_g[_j].k+1./mol->ff_g[_k].k; //Because _d is then multiplied by k we dont need kJ/deg -> kJ/rad conversion
    mol->ff_g[_i].v+=a.alpha/(mol->ff_g[_i].k*_d), mol->ff_g[_j].v+=a.alpha/(mol->ff_g[_j].k*_d), mol->ff_g[_k].v+=a.alpha/(mol->ff_g[_k].k*_d);
    }

//Stage II. Remove non-existing dihedrals (i.e. at open angles) X0 <- C_=_C -> X1
//Stage II.1. Remove impropers
_i=mol->size_i;
while (_i--)
  if ( (mol->ff_i[_i].type))
    {
...
//Stage III. Set all out-of-cycles to their initial isomer values
if ( (r))
  {
  _i=mol->size_i;
  while (_i--)
...
if ( (verbose))
  ...
  if ((_i=classify_cycles(order,neighbors,cycles,str))==(unsigned int)-1) 
    ...
    {
    yprintf(YPRINTF_INFO,"classify_cycles() has detected %d aromatic rings.\n",_i);
    if ( (_i))
      {
      if (update_aromatic_cycles_bonds(order,neighbors,_i,cycles,str)==(unsigned int)-1) { yprintf(YPRINTF_ERROR,"error in update_aromatic_cycles_bonds(), err_code=%s.\n",get_yerrno(ylib_errno)); goto LABEL_ERROR_2; }
      else yprintf(YPRINTF_INFO,"update_aromatic_cycles_bonds() has edited the bonds in aromatic rings, i.e. bond type '1'->'a'.\n");
      }
    }
  ...
  if ((_i=classify_cycles(order,neighbors,cycles,str))==(unsigned int)-1) goto LABEL_ERROR_2; 
  else { if ( ( (_i))&&(update_aromatic_cycles_bonds(order,neighbors,_i,cycles,str)==(unsigned int)-1) ) goto LABEL_ERROR_2; } 
  //First order compiler
...
{
register unsigned int _i, _j, _k, _l, _t;
unsigned int res_name, atom_num, edge_num, *up;
...
for (_i=0; _i<adjacency->size_v; _i++)
  ...
  if (!(up=calc_brutto_f(sstr.natoms,sstr.a))) { LABEL_MEMORY_ERROR_7: if ( (sstr.nedges)) free(sstr.edges), sstr.edges=0x0; goto LABEL_MEMORY_ERROR_6; }
  _j=print_brutto_f(&brt_f,up); free(up), up=0x0; if (_j==(unsigned int)-1) goto LABEL_MEMORY_ERROR_7;
...
for ( brr_size=0, _i=0; _i<adjacency->size_v; _i++)
  for ( _k=0; _k<adjacency->nn[_i]; _k++)
    if ((_j=adjacency->nv[_i][_k])>_i)
      ...
      if (!(up=calc_brutto_f(cstr.natoms,cstr.a))) { LABEL_MEMORY_ERROR_8: free(cstr.edges), cstr.edges=0x0; goto LABEL_MEMORY_ERROR_7; }
      _j=print_brutto_f(&brt_f,up); free(up), up=0x0; if (_j==(unsigned int)-1) goto LABEL_MEMORY_ERROR_8;
